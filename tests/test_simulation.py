...
class TestDensity(unittest.TestCase):
    ...
    def test_rectangular_deposition_charge_and_corners(self):
        ...
        np.testing.assert_allclose(rho.sum() * np.prod(delta_r), charges.sum())
        ...
        np.testing.assert_allclose(wrapped.sum() * np.prod(delta_r), charges.sum())
...
class TestEnergy(unittest.TestCase):
    def test_data(self):
        ...
        self.assertGreater(len(phase_files), 0)
    ...
    def test_field_energy_physical_grid_independence(self):
        ...
        for n in (np.array([8, 12]), np.array([16, 24]), np.array([4, 6])):
            ...
            energies.append(calculate_field_energy(rho, phi, delta_r))
